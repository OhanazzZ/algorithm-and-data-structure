...
wang (BIIM f g : left) right reducedLeft reducedRight
  = wang (OR (NOT f) g : OR (NOT g) f : left) right reducedLeft reducedRight
wang left (BIIM f g : right) reducedLeft reducedRight
  = wang left (AND f g : AND (NOT f) (NOT g) : right) reducedLeft reducedRight


-- Base case (copy over your solution from the first problem).
